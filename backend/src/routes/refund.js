...
router.post('/verify', upload.single('image'), async (req, res) => {
  try {
    ...
    if (isAi || trustScore < 50 || exifResults.flags.length > 1) {
      riskLevel = 'HIGH';
      action = 'DENY';
    } else if (trustScore < 80 || exifResults.flags.length > 0 || aiScore > 0.3) {
      riskLevel = 'MEDIUM';
    ...
    const result = {
      ...
      signal_breakdown: {
        ...
        trust_score_check: { result: trustScore < 80 ? "FAIL" : "PASS", score: trustScore, prior_flags: priorFlags }
      }
    };

    // Note: Logging should happen asynchronously or before returning
